<template>
  <div class="hot-mv-div">
    ...
    <el-carousel :interval="5000" arrow="always" class="hot-mv-swipe">
      <el-carousel-item v-for="item in hot_mv?Math.ceil(hot_mv.length/10):0" :key="item">
        <!-- <h3>{{ item.title }}</h3> -->
        <ul class="swipe-ul">
          <li v-for="(MV, index) in hot_mv?hot_mv.slice((item-1)*10,item*10):[]" :key="index">
            <img class="swipe-img" :src="MV.cover" alt />

            <p class="m-title">
              {{MV.title}}
              <span style="color:orange">{{MV.rate}}</span>
            </p>
          </li>
        </ul>
      </el-carousel-item>
